...
import { testSpots, testTracks, beethoven, seaSpot, concerto, testUsers } from "../fixtures.js";
...
suite("Track Model tests", () => {
  ...
  test("create single track", async () => {
    const seaSpotList = await db.spotStore.addspot(seaSpot);
    const track = await db.trackStore.addTrack(seaSpotList._id, concerto);
    assert.isNotNull(track._id);
    assertSubset(concerto, track);
  });

  test("create multiple trackApi", async () => {
    const tracks = await db.spotStore.getspotById(beethovenList._id);
  ...
  test("delete One Track - success", async () => {
    ...
    const tracks = await db.trackStore.getAllTracks();
    assert.equal(tracks.length, testSpots.length - 1);
  ...
  test("delete One User - fail", async () => {
    ...
    const tracks = await db.trackStore.getAllTracks();
    assert.equal(tracks.length, testSpots.length);
  });
});
